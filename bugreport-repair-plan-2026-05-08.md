...
- 考试答题入口：答题页刷新/开始答题参数改为安全解析，异常题库 id 或角色 id 提示后退出；普通题库角色过滤空值、负数和越界值，两类人员默认角色 id 显式对齐接口参数。
...
- 岗前培训考试：课程完成上报改走 `api/before/subjectstudy`，按返回的 `training_exams_id` 进入结束人脸；结束人脸后只在 `faceType=end` 时跳签字/考试，岗前签字上传后补提交签字并继续进入考试。
