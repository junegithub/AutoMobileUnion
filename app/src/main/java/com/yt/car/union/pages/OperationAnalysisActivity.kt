...
class OperationAnalysisActivity : AppCompatActivity() {
    ...
    private fun initVehiclePieData(markerView: CustomMarkerView) {
        ...
        val entries = mutableListOf<PieEntry>().apply {
            // 正常（72.77%）
            add(PieEntry(10f, getString(R.string.normal)).apply { data = getString(R.string.normal)})
            // 欠费（27.23%，绑定点击数据）
            add(PieEntry(20f, getString(R.string.arrears)).apply { data = getString(R.string.arrears) })
            // 维修/停用（0%，自动隐藏）
            add(PieEntry(30f, getString(R.string.maintenance)).apply { data = getString(R.string.maintenance) })
            add(PieEntry(40f, getString(R.string.stop)).apply { data = getString(R.string.stop) })
        }
    ...
    private fun initAlarmPieData(markerView: CustomMarkerView) {
        val entries = mutableListOf<PieEntry>().apply {
            // 其他报警（90.77%）
            add(PieEntry(30f, getString(R.string.other_alarm)).apply { data = getString(R.string.other_alarm) })
            // 超速报警（4.30%，绑定点击数据）
            add(PieEntry(30f, getString(R.string.overspeed_alarm)).apply { data = getString(R.string.overspeed_alarm) })
            // 疲劳驾驶（0.51%）
            add(PieEntry(30f, getString(R.string.fatigue_driving)).apply { data = getString(R.string.fatigue_driving) })
            // 摄像头故障（4.43%）
            add(PieEntry(10f, getString(R.string.camera_fault)).apply { data = getString(R.string.camera_fault) })
        }
